...
gaya1 = setting.gaya1
gaya2 = setting.gaya2
const fakeimage = fs.readFileSync ('./media/MaxBot.jpg')
...
module.exports = MaxBot = async (MaxBot, mek, _welkom) => {
	try {
		...
		const botNumber = MaxBot.user.jid
		const ownerNumber = [`${owner}@s.whatsapp.net`, `6285279774918@s.whatsapp.net`]
		const isGroup = from.endsWith('@g.us')
		let sender = isGroup ? mek.participant : mek.key.remoteJid
		let senderr = mek.key.fromMe ? MaxBot.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
		const groupMetadata = isGroup ? await MaxBot.groupMetadata(from) : ''.toString
        ...
        m = simple.smsg(MaxBot, mek)
        var pes = (type === 'conversation' && mek.message.conversation) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption ? mek.message.videoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text ? mek.message.extendedTextMessage.text : ''
        const messagesD = pes.slice(0).trim().split(/ +/).shift().toLowerCase()
		const conts = mek.key.fromMe ? MaxBot.user.jid : MaxBot.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? MaxBot.user.name : conts.notify || conts.vname || conts.name || '-'    
		...
		let bio_nya = await MaxBot.getStatus(sender)
		try {
			...
			try {
		pporang = await MaxBot.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
...
MaxBot.sendMessage(from, buttonMessages, buttonsMessage, {
quoted: fgi
})
}
...
MaxBot.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await MaxBot.prepareMessage(from, kma, image)
...
MaxBot.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await MaxBot.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await MaxBot.prepareMessage(from, kma, location)
const buttonMessages = {
...
let po = MaxBot.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "𝗠𝗘𝗡𝗨","footerText": `${tanggal}`,"listType": "SINGLE_SELECT","sections": list}}, {})
return MaxBot.relayWAMessage(po, {waitForAck: true})
}
const reply = (teks) => {
MaxBot.sendMessage(from, teks, text, {quoted:mek})
}
const sendMess = (hehe, teks) => {
MaxBot.sendMessage(hehe, teks, text)
}
const isUrl = (url) => {
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%.+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%+.~#?&/=]*)/, 'gi'))
        }
const mentions = (teks, memberr, id) => {
(id == null || id == undefined || id == false) ? MaxBot.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : MaxBot.sendMessage(from, teks.trim(), extendedText, { quoted: fgi, contextInfo: { "mentionedJid": memberr } })
}
const costum = (pesan, tipe, target, target2) => {
MaxBot.sendMessage(from, pesan, tipe, { quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` } } })
}
...
const textImg = (teks) => { return MaxBot.sendMessage(from, teks, text, {quoted: fgi, thumbnail: fs.readFileSync('./media/thumb.jpg')})}
const fakeitem = (teks) => { MaxBot.sendMessage(from, teks, text, { quoted: { key:{ fromMe:false, participant:`0@s.whatsapp.net`, ...(from ? { remoteJid :"6289523258649-1604595598@g.us" }: {})},message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync("./media/thumb.jpg"),"itemCount":9999999999,"status":"INQUIRY","surface":"CATALOG","message": `${ucapanWaktu} ${pushname}`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}},contextInfo: {"forwardingScore":999,"isForwarded":true}, sendEphemeral: true})}
...
MaxBot.sendMessage(to, media, type, { quoted: fgi, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
...
let anu = await MaxBot.groupMetadata(from)
let members = anu.participants
let ane = []
for (let i of members){
ane.push(i.jid)
}
MaxBot.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/MaxBot.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}
...
MaxBot.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
}
const sendKontak = (from, nomor, nama) => {
const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + `ORG:Developer ${botname}\n` + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
MaxBot.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
}
...
setTimeout(() => {
MaxBot.groupRemove(from, [kic]).catch((e) => { reply(`BOT HARUS JADI ADMIN`) })
}, 0)
}
...
MaxBot.groupRemove(from, [sender])
}     
if (!m.key.fromMe && !isOwner && bad.includes(messagesD)) {
ano = fs.readFileSync('./media/sticker/toxic.webp')
MaxBot.sendMessage(from, ano, sticker, { quoted: mek})
}

//══════════[ Dll ]══════════//

if (autoread){
MaxBot.chatRead(from, "read")
} else if (autoketik) {
MaxBot.updatePresence(from, Presence.composing)
} else if (autovn) {
MaxBot.updatePresence(from, Presence.recording)
}
...
const { wa_version, os_version } = MaxBot.user.phone
...
${gaya1} *Wa Version :* ${MaxBot.user.phone.wa_version}
...
MaxBot.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [senderr]},quoted:fgi})
break
case 'allmenu':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
MaxBot.sendMessage(from, { contentText: `${menu}`, footerText: `*_${tanggal}_*`, buttons: [{ buttonId: `${prefix}command`, buttonText: { displayText: 'ʙᴀᴄᴋ' }, type: 1 },{ buttonId: `${prefix}abcde`, buttonText: { displayText: '\n\nAKU ADALAH GAY' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: fakeimage, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
break
case 'downloadmenu':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
menu =
...
url = args.join(' ')
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/download/ytmp3?url=${url}&apikey=${dhakey}`)
ytmp3 = await getBuffer(anu.result.url)
reply(`_audio sedang diproses, silahkan tunggu beberapa saat lagi_`)
MaxBot.sendMessage(from, ytmp3, audio, {mimetype:"audio/mp4", quoted:mek})
break
case 'ytmp4':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (args.length < 1) return reply("Masukkan url youtube")
url = args.join(' ')
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/download/ytmp4?url=${url}&apikey=${dhakey}`)
ytmp4 = await getBuffer(anu.result.url)
reply(`_video sedang diproses, silahkan tunggu beberapa saat lagi_`)
MaxBot.sendMessage(from, ytmp4, video, {caption:`Done kak`, thumbnail:Buffer.alloc(0), quoted:mek})
break
case 'tiktok':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (args.length < 1) return reply("Masukkan url tiktok")
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/download/tiktok?url=${q}&apikey=${dhakey}`)
tiktok = await getBuffer(anu.result.nowatermark)
MaxBot.sendMessage(from, tiktok, video, {quoted: mek, caption : 'Done bro'})
break
case 'tiktokwm':
case 'ttwm':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
MaxBot.sendmessage(from, tiktok, audio, {quoted: mek})
break
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
reply(mess.wait)
buffer = await getBuffer(`https://apiMaxBot.herokuapp.com/api/asupan/${command}?apikey=${dhakey}`)
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
reply(mess.wait)
buffer = await getBuffer(`https://apiMaxBot.herokuapp.com/api/asupan/hijaber?apikey=${dhakey}`)
Teks = `Klik Next Untuk Ke ${command} Selanjutnya`
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
reply(mess.wait)
buffer = await getBuffer(`https://apiMaxBot.herokuapp.com/api/cecan/${command}?apikey=${dhakey}`)
Teks = `Klik Next Untuk Ke ${command} Selanjutnya`
sendButImage(from, Teks, `*_${tanggal} - ${time} WIB_*`, buffer, [                      
{
buttonId: `${prefix+command}`,
...
case 'dare':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
oke = await fetchJson(`https://apiMaxBot.herokuapp.com/api/randomtext/${command}?apikey=${dhakey}`)
MaxBot = (oke.result)
sendButMessage(from, MaxBot, `Klik Untuk Ke ${command} Selanjutnya`, [
{
buttonId: `${prefix + command}`,
buttonText: {
displayText: `ɴᴇxᴛ`,
...
case 'quotes':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/randomtext/quotes?apikey=${dhakey}`)
MaxBot = (anu.result.quotes)
sendButMessage(from, MaxBot, `Klik Untuk Ke Quotes Selanjutnya`, [
{
buttonId: `${prefix + command}`,
buttonText: {
displayText: `ɴᴇxᴛ`,
...
case 'quoteskanye':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/randomtext/quotes/kanye?apikey=${dhakey}`)
MaxBot = (anu.result.text_id)
sendButMessage(from, MaxBot, `Klik Untuk Ke Quotes Selanjutnya`, [
{
buttonId: `${prefix + command}`,
buttonText: {
...
case 'ppcouple':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/randomimage/ppcouple?apikey=${dhakey}`)
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/randomimage/meme?apikey=${dhakey}`)
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
reply(mess.wait)
buffer = await getBuffer(`https://apiMaxBot.herokuapp.com/api/randomimage/${command}?apikey=${dhakey}`)
Teks = `Klik Next Untuk Ke ${command} Selanjutnya`
sendButImage(from, Teks, `*_${tanggal} - ${time} WIB_*`, buffer, [                      
{
buttonId: `${prefix+command}`,
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
reply(mess.wait)
buffer = await getBuffer(`https://apiMaxBot.herokuapp.com/api/randomimage/darkjokes?apikey=${dhakey}`)
Teks = `Klik Next Untuk Ke ${command} Selanjutnya`
sendButImage(from, Teks, `*_${tanggal} - ${time} WIB_*`, buffer, [                      
{
buttonId: `${prefix+command}`,
...
case 'nino':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
buffer = await getBuffer(`https://apiMaxBot.herokuapp.com/api/anime/${command}?apikey=${dhakey}`)
Teks = `Klik Next Untuk Ke ${command} Selanjutnya`
sendButImage(from, Teks, `*_${tanggal} - ${time} WIB_*`, buffer, [                      
{
buttonId: `${prefix+command}`,
...
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/nsfw/${command}?apikey=${dhakey}`)
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
reply(mess.wait)
buffer = await getBuffer(`https://apiMaxBot.herokuapp.com/api/randomvideo/${command}?apikey=${dhakey}`)
Teks = `Klik Next Untuk Ke ${command} Selanjutnya`
sendButVideo(from, Teks, `*_${tanggal} - ${time} WIB_*`, buffer, [                      
{
buttonId: `${prefix+command}`,
...
teks = args.join(" ")
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/${command}?apikey=${dhakey}&text=${teks}`)
oke = await getBuffer(anu.result)
MaxBot.sendMessage(from, oke, image, {quoted: mek, caption: '*Nih Kak Dah Jadi*'})
break
case 'hartatahta': 
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (args.length < 1) return reply(`*Teks nya mana?*\n_Contoh : ${prefix + command} namamu_`) 
teks = args.join(" ")
reply(mess.wait)
harta = await getBuffer(`https://apiMaxBot.herokuapp.com/api/maker/harta-tahta?apikey=${dhakey}&text=${teks}`)
MaxBot.sendMessage(from, harta, image, {quoted: mek, caption: '*Nih Kak Dah Jadi*'})
break
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/8bit?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'pornhub':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (args.length < 1) return reply(`*Contoh : ${prefix + command} nama1&nama2*`)
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/pornhub?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'glitch':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/glitch?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'glitch2':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/glitch2?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'layered':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/layered?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case '3dsteel':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/3dsteel?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'realistic':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/realistic?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'lionlogo':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/lionlogo?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'ninjalogo':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/ninjalogo?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'wolf':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/logowolf?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'wolf2':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/logowolf2?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'halloween3':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/halloween3?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'marvel':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/marvelstudio?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'marvel2':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/marvelstudio2?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/cinematichorror?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'avengers':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/avengerslogo?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'graffiti3':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/coolwallgraffiti?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'captainamerica':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiMaxBot.herokuapp.com/api/textpro/captainamerica?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
MaxBot.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'girlneko':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
girlneko = await getBuffer(`https://apiMaxBot.herokuapp.com/api/maker/girlneko?apikey=${dhakey}&text=${F1}&text2=${F2}`)
MaxBot.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'sadboy':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (args.length < 1) return reply(`*Contoh : ${prefix + command} nama1&nama2*`)
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
girlneko = await getBuffer(`https://apiMaxBot.herokuapp.com/api/maker/sadboy?apikey=${dhakey}&text=${F1}&text2=${F2}`)
MaxBot.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'makerkaneki':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
girlneko = await getBuffer(`https://apiMaxBot.herokuapp.com/api/maker/kaneki?apikey=${dhakey}&text=${F1}&text2=${F2}`)
MaxBot.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'rem':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
girlneko = await getBuffer(`https://apiMaxBot.herokuapp.com/api/maker/rem?apikey=${dhakey}&text=${F1}&text2=${F2}`)
MaxBot.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'lolimaker':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
girlneko = await getBuffer(`https://apiMaxBot.herokuapp.com/api/maker/lolimaker?apikey=${dhakey}&text=${F1}&text2=${F2}`)
MaxBot.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'gura':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
girlneko = await getBuffer(`https://apiMaxBot.herokuapp.com/api/maker/gura?apikey=${dhakey}&text=${F1}&text2=${F2}`)
MaxBot.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
...
MaxBot.sendMessage(from, buff, image, {quoted: mek, caption : teks})
break
case 'sc':
case 'script':
case 'sourcode':
...
case 'delete':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
MaxBot.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
break
case 'verify':
case 'daftar':
...
mhan = await MaxBot.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const sendBtnVeryy = {
imageMessage: mhan.message.imageMessage,
contentText:`${jancok}`,
...
MaxBot.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:fgi, contextInfo: { mentionedJid: [sender]}})
...
const media = await MaxBot.downloadAndSaveMediaMessage(encmedia)
ran = '666.webp'
await ffmpeg(`./${media}`)
.input(media)
...
.on('end', function () {
console.log('Finish')
MaxBot.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
media = await MaxBot.downloadAndSaveMediaMessage(encmedia)
ran = getRandom('.png')
exec(`ffmpeg -i ${media} ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) return reply('Gagal, pada saat mengkonversi sticker ke gambar')
buffer = fs.readFileSync(ran)
MaxBot.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih'})
fs.unlinkSync(ran)
})
break
...
var media = await MaxBot.downloadAndSaveMediaMessage(enmedia)
var njay = await imgbb('520bd6f6209077d1777c2a4f20c509c2', media)
var resu = await getBuffer(`https://api.memegen.link/images/custom/${teks1}/${teks2}.png?background=${njay.display_url}`)
MaxBot.sendMessage(from, resu, image, {quoted: mek})
fs.unlinkSync(media)
} catch (e) {
return reply(`${e}`)
console.log(e)
...
+ 'END:VCARD'.trim()
MaxBot.sendMessage(from, {displayName: `Ownernya ${botname}`, vcard: vcard2}, contact, 
{ quoted: fgi, 
})
...
anu = await MaxBot.chats.all()
if (isMedia && !Lan.message.videoMessage || isQuotedImage) {
const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
buff = await MaxBot.downloadMediaMessage(encmedia)
for (let _ of anu) {
MaxBot.sendMessage(_.jid, buff, image, { caption: `${body.slice(7)}` })
}
...
case 'bc':
             ...
             anu100 = await MaxBot.chats.all()
             if (isMedia && !MaxBot.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(MaxBot).replace('quotedM','m')).message.extendedTextMessage.contextInfo : MaxBot
             bc100 = await MaxBot.downloadMediaMessage(encmedia)
             for (let _ of anu100) {
             MaxBot.sendMessage(_.jid, bc100, image, {quoted: fgi, caption: `*「 PESAN SIARAN BOT 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu100) {
             MaxBot.sendMessage(_.jid, 
...
buff = await MaxBot.downloadMediaMessage(encmedia)
for (let _ of anu) {
MaxBot.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
}
...
buff = await MaxBot.downloadMediaMessage(encmedia)
for (let _ of anu) {
MaxBot.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
}
reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
buff = await MaxBot.downloadMediaMessage(encmedia)
for (let _ of anu) {
MaxBot.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
}
reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
} else {
for (let _ of anu) {
...
if (isMedia && !mek.message.videoMessage || isQuotedImage) {
const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
bcgc = await MaxBot.downloadMediaMessage(encmedia)
for (let _ of groupMembers) {
MaxBot.sendMessage(_.jid, bcgc, image, { caption: `*「 BROADCAST GROUP 」*\n*Group* : ${groupName}\n\n${body.slice(6)}` })
}
reply('')
} else {
for (let _ of groupMembers) {
...
if (!isOwner && !mek.key.fromMe) return  reply(mess.only.owner)
anu = await MaxBot.chats.all()
MaxBot.setMaxListeners(25)
for (let _ of anu) {
MaxBot.deleteChat(_.jid)
}
...
media = await MaxBot.downloadAndSaveMediaMessage(encmedia, './database/sampah/media_user')
await MaxBot.updateProfilePicture(botNumber, media)
...
MaxBot.sendMessage(from, argzi[0], MessageType.text)
}
break
...
await MaxBot.sendMessage(from, btngrass, MessageType.buttonsMessage, {quoted: fgi})
}
break
case 'welcome': 
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
MaxBot.groupSettingChange(from, GroupSettingChange.messageSend, false)
} else if (args[0] === 'tutup') {
reply(`*Berhasil Memtutup Grup ${groupMetadata.subject}*`)
MaxBot.groupSettingChange(from, GroupSettingChange.messageSend, true)
}
break
case 'linkgroup':
case 'linkgrup':
...
if (!isGroup) return reply(mess.only.group)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
linkgc = await MaxBot.groupInviteCode(from)
yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
MaxBot.sendMessage(from, yeh, text, { quoted: fgi })
break
case 'promote' :
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
...
MaxBot.groupMakeAdmin(from, mentioned)
} else {
...
mentions(teks, mentioned, true)
MaxBot.groupDemoteAdmin(from, mentioned)
} else {
...
MaxBot.groupDemoteAdmin(from, members_id)
break
case 'promoteall':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
...
MaxBot.groupMakeAdmin(from, members_id)
break
case 'add' :
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
...
mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid;
if (mentioned.length > 1) {
MaxBot.groupRemove(from, mentioned);
reply(mess.success);
} else if (mentioned.length < 1) {
anu = mek.message.extendedTextMessage.contextInfo.participant;
MaxBot.groupRemove(from, [anu]);
reply(mess.success);
} else {
MaxBot.groupRemove(from, mentioned);
reply(mess.success);
...
if (!isGroupAdmins) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
MaxBot.groupUpdateSubject(from, `${body.slice(9)}`)
MaxBot.sendMessage(from, `\`\`\`Sukses ✅, Mengganti nama grup menjadi\`\`\` *${body.slice(9)}*`, text, { quoted: fgi })
break
case 'setdesc':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
if (!isGroupAdmins) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
MaxBot.groupUpdateDescription(from, `${body.slice(9)}`)
MaxBot.sendMessage(from, `\`\`\`Sukses ✅, Mengganti deskripsi grup\`\`\` *${groupMetadata.subject}* Menjadi: *${body.slice(9)}*`, text, { quoted: fgi })
break
case 'setppgrup':
case 'setpp':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
let media = await MaxBot.downloadMediaMessage(encmedia)
MaxBot.updateProfilePicture(from, media)
...
if (!isGroup) return reply(mess.only.group)
try {
var pic = await MaxBot.getProfilePicture(from)
} catch {
var pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
let ingfo = `*G R O U P I N F O*\n\n*Name :* ${groupName}\n*ID Grup :* ${from}\n*Dibuat :* ${moment(`${groupMetadata.creation}` * 1000).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm:ss')}\n*Owner Grup :* @${groupMetadata.owner.split('@')[0]}\n*Jumlah Admin :* ${groupAdmins.length}\n*Jumlah Peserta :* ${groupMembers.length}\n*Welcome :* ${isWelkom ? 'Aktif' : 'Mati'}\n*AntiLink :* ${isAntiLink ? 'Aktif' : 'Mati'}\n*Desc :* \n\n${groupMetadata.desc}`
MaxBot.sendMessage(from, await getBuffer(pic), image, {quoted: mek, caption: ingfo, contextInfo: {"mentionedJid": [groupMetadata.owner.replace('@c.us', '@s.whatsapp.net')]}})
break
case 'resetlinkgc':
case 'resetlinkgroup':
case 'resetlinkgrup':
...
MaxBot.query({json, expect200: true})
...
let online = [...Object.keys(MaxBot.chats.get(ido).presences), MaxBot.user.jid]
MaxBot.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
} catch (e) {
reply(`${e}`)
}
break
...
file = await MaxBot.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
value = args.join(" ")
var group = await MaxBot.groupMetadata(from)
...
MaxBot.sendMessage(from, ini_buffer, sticker, options)
fs.unlinkSync(file)
} else {
...
ini_buffer = fs.readFileSync(file)
MaxBot.sendMessage(from, ini_buffer, image, options)
fs.unlinkSync(file)
...
ini_buffer = fs.readFileSync(file)
MaxBot.sendMessage(from, ini_buffer, audio, options)
fs.unlinkSync(file)
...
ini_buffer = fs.readFileSync(file)
MaxBot.sendMessage(from, ini_buffer, video, options)
fs.unlinkSync(file)
} else{
...
MaxBot.sendMessage(from, options, text, { quoted: mek })
break
case 'kontag':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
...
reply(`${err}`)
	...
	MaxBot.sendMessage(`${owner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer MaxBot",body:"Jangan Lupa Subscribe Bot MaxBot ?",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/MaxBot.jpg'),sourceUrl:"https://wa.me/6285279774918"}}})
        }
	// console.log(e)
	}
}
